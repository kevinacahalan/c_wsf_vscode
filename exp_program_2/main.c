...
int n_regions = 3;
int n_trainers = 3;
int min_monsters_in_region = 3;
int max_monsters_in_region = 10;
int min_captures = 3;
int max_captures = 6;
int random_spaces_average_spacing = 9; //0 for no random spaces, bigger numbers mean less
...
int main(int argc, char const *argv[]) {
    ...
    for (int i = 0; i < n_monsters; i++) {
        random_space(ofp, random_spaces_average_spacing);
    ...
    for (int i = 0; i < n_regions; i++) {
        int monsters_in_region = min_monsters_in_region + (rand() % (1 + max_monsters_in_region - min_monsters_in_region));
        fprintf(ofp, "\nREGI_%d\n", 1 + i);
        random_space(ofp, random_spaces_average_spacing);
        fprintf(ofp, "%d monsters\n", monsters_in_region);
        for (int j = 0; j < monsters_in_region; j++) {
            //can end up with more then 1 monster of same type in region, this maybe should change
            int random_1_to_n_monsters = 1 + (rand() % n_monsters);
            fprintf(ofp, "MONST_%d\n", random_1_to_n_monsters);
            random_space(ofp, random_spaces_average_spacing);
        }
    }
    ...
    for (int i = 0; i < n_trainers; i++) {
        ...
        for (int j = 0; j < regions_in_trainer; j++) {
            int separation = n_regions / regions_in_trainer;
            random_space(ofp, random_spaces_average_spacing);
            fprintf(ofp, "REGI_%d\n", 1 + (j * separation));
        }
    }
